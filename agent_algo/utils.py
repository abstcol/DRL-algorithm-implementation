...
class Gaussion_Net(nn.Module):
    ...
    def __init__(self, n_state, n_action, hidden_sizes=args.hidden_sizes, final_ac=None):
        ...
        """
        super(Gaussion_Net, self).__init__()
        ...
        # 输出层
        self.mu = nn.Linear(input_size, n_action)  # 输出动作均值
        self.sigma = nn.Linear(input_size, n_action)  # 输出动作标准差
    ...
    def forward(self, x):
        ...
        mu=self.mu(x)
        sigma=self.sigma(x)
        sigma = torch.clamp(sigma, min=-5, max=2)  # 限制 sigma 范围
        sigma = torch.exp(sigma)  # 确保方差为正
        return mu,sigma
...
class Net(nn.Module):
    ...
    def forward(self,x):
        ...
        前向传播
        :param x:输入observation，形状为(batch_size,n_state)
        :return:输出为指定状态时各个动作的q value 形状为(batch_size,n_action)
        """
...
class MyDeque_Continuous():
    def __init__(self,maxlength,obs_size,batch_size,action_size):
        """
        ...
        self.memory_action = torch.zeros((maxlength, action_size), dtype=torch.long, device=device)
    ...
    def get_index(self):
        ...
        """
        if self.full==True:
            return torch.multinomial(self.memory_weights,self.batch_size,replacement=False)
        return torch.multinomial(self.memory_weights[:self.idx],self.batch_size,replacement=False)
...
class MyDeque_AC():
    def __init__(self,maxlength,obs_size,batch_size):
        """
        ac方法offpolicy实现专用，主要区别在于需要额外存储但是action的概率用于重要性采样
        自定义的经验放回区，用于存储和采样经验，是一个循环队列结构，实质上直接使用dequeue也可实现，这里单独实现主要有两个目的：
        1.功能封装，代码更为简介
        2.想要直接将经验存储为tensor形式且保存在gpu中，减少cpu和gpu之间的通信次数，加速训练速度（事实证明没有用）
        :param maxlength:经验池的大小
...
class MyDeque_Onpolicy():
    def __init__(self,maxlength,obs_size,vec_num,action_size):
        """
        自定义的经验放回区，用于存储和采样经验，是一个循环队列结构，实质上直接使用dequeue也可实现，这里单独实现主要有两个目的：
        1.功能封装，代码更为简介
        2.想要直接将经验存储为tensor形式且保存在gpu中，减少cpu和gpu之间的通信次数，加速训练速度（事实证明没有用）
        :param maxlength:多少步更新一次
        :param obs_size:状态空间的size
        :param vec_num:每一次采样的经验数量（并行数）
        """

        self.maxlength=maxlength
        self.vec_num=vec_num
        self.batch=self.maxlength*self.vec_num
        #初始化存储经验的张量
        self.memory_obs = torch.zeros((self.batch, obs_size), dtype=torch.float32, device=device)
        self.memory_action = torch.zeros((self.batch, action_size), dtype=torch.float32, device=device)
        self.memory_reward = torch.zeros((self.batch, 1), dtype=torch.float32, device=device)
        self.memory_next_obs = torch.zeros((self.batch, obs_size), dtype=torch.float32, device=device)
        self.memory_terminated = torch.zeros((self.batch, 1), dtype=torch.bool, device=device)
    ...
    def get_batch(self):
        ...
        """
        self.idx=0
        return self.memory_obs,self.memory_action,self.memory_reward,self.memory_next_obs,self.memory_terminated
    ...
    def append(self,obs,action,reward,next_obs,terminated):
        ...
        """
        self.memory_obs[self.idx]=torch.tensor(obs, dtype=torch.float32, device=device)
        self.memory_action[self.idx]=torch.tensor(action, dtype=torch.float32, device=device)
        self.memory_reward[self.idx]=torch.tensor(reward, dtype=torch.float32, device=device)
        self.memory_next_obs[self.idx]=torch.tensor(next_obs, dtype=torch.float32, device=device)
        self.memory_terminated[self.idx] = torch.tensor(terminated, dtype=torch.bool, device=device)
